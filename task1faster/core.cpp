    ...
    for (const char* doc_str_iterator = doc_str; *doc_str_iterator; doc_str_iterator++) {
        if (*doc_str_iterator == ' ') {
            ...
            for (auto query_element_iterator = queries.begin(); query_element_iterator != queries.end(); query_element_iterator++) {
                ...
                int dist = 4;
                na = std::strlen(query_element_iterator->query_str);
                switch (query_element_iterator->match_type) {
                case MT_EXACT_MATCH:
                    dist = hamming_distance(query_element_iterator->query_str, b, na, nb, 0) == 0 ? 0 : 4;
                    break;
                case MT_HAMMING_DIST:
                    dist = hamming_distance(query_element_iterator->query_str, b, na, nb, query_element_iterator->match_dist + 1);
                    break;
                case MT_EDIT_DIST:
                    dist = edit_distance(query_element_iterator->query_str, b, na, nb, query_element_iterator->match_dist + 1);
                    break;
                }
